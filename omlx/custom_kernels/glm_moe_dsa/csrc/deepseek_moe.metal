...
template <
    ...
    int WM,
    int WN>
[[kernel]] void deepseek_mxfp4_gather_masked_row_rhs(
    const device T* x [[buffer(0)]],
    const device uint32_t* w [[buffer(1)]],
    const device uint8_t* scales [[buffer(2)]],
    const device uint32_t* indices [[buffer(3)]],
  ...
  if (!route_mask[row] || expert < 0 || expert >= E) {
    const uint lane = simd_group_id * SIMD_SIZE + simd_lane_id;
    const uint threads = WM * WN * SIMD_SIZE;
    for (int col = int(lane); col < min(BN, N - y_col); col += int(threads)) {
      yl[col] = T(0);
    }
    return;
  }
  ...
  const size_t stride_s = size_t(N) * K_g;
  const short tgp_bn = short(min(BN, N - y_col));
  const short tgp_bm = 1;
  const short2 tile_x = short2(k_remain, tgp_bm);
  const short2 tile_w = short2(k_remain, tgp_bn);

  const device T* xl = x + size_t(source_row) * K;
  const device uint8_t* wl =
      ((const device uint8_t*)w) + size_t(expert) * stride_w +
      size_t(y_col) * K_w;
  const device uint8_t* sl =
...
#define instantiate_deepseek_mxfp4_masked_row(type, bm, bn, bk, wm, wn)        \
  instantiate_kernel(                                                          \
      "deepseek_mxfp4_gather_masked_row_rhs_" #type "_bm_" #bm "_bn_" #bn  \
      "_bk_" #bk "_wm_" #wm "_wn_" #wn,                                  \
      deepseek_mxfp4_gather_masked_row_rhs,                                    \
      type,                                                                    \
      bm,                                                                      \
      bn,                                                                      \
      bk,                                                                      \
...
instantiate_deepseek_mxfp4_masked_row(float16_t, 8, 32, 32, 1, 2);
instantiate_deepseek_mxfp4_masked_row(float16_t, 16, 32, 32, 1, 2);
instantiate_deepseek_mxfp4_masked_row(float16_t, 32, 32, 32, 1, 2);
instantiate_deepseek_mxfp4_masked_row(float16_t, 16, 64, 32, 1, 2);
instantiate_deepseek_mxfp4_masked_row(float16_t, 32, 64, 32, 1, 2);
instantiate_deepseek_mxfp4_masked_row(bfloat16_t, 8, 32, 32, 1, 2);
instantiate_deepseek_mxfp4_masked_row(bfloat16_t, 16, 32, 32, 1, 2);
instantiate_deepseek_mxfp4_masked_row(bfloat16_t, 32, 32, 32, 1, 2);
instantiate_deepseek_mxfp4_masked_row(bfloat16_t, 16, 64, 32, 1, 2);
instantiate_deepseek_mxfp4_masked_row(bfloat16_t, 32, 64, 32, 1, 2);
