...
template <typename T>
[[kernel]] void deepseek_mxfp4_gather_masked_pair_qmv(
    const device T* x [[buffer(0)]],
    const device uint32_t* w0 [[buffer(1)]],
    const device uint8_t* scales0 [[buffer(2)]],
    const device uint32_t* w1 [[buffer(3)]],
    ...
    const device bool* route_mask [[buffer(6)]],
    device T* y [[buffer(7)]],
    const constant int& T_rows [[buffer(8)]],
    const constant int& M [[buffer(9)]],
    const constant int& N [[buffer(10)]],
    const constant int& K [[buffer(11)]],
    const constant int& E [[buffer(12)]],
    uint3 tid [[threadgroup_position_in_grid]],
    uint simd_group_id [[simdgroup_index_in_threadgroup]],
    uint simd_lane_id [[thread_index_in_simdgroup]]) {
  constexpr int outputs_per_group = 8;
  ...
  if (!route_mask[row] || expert < 0 || expert >= E) {
    if (simd_lane_id == 0) {
      for (int i = 0; i < outputs_per_group / 2 && out_row + i < N; ++i) {
        y[size_t(row) * (2 * N) + out_row + i] = T(0);
        y[size_t(row) * (2 * N) + N + out_row + i] = T(0);
      }
    }
    return;
  }
  ...
  const size_t scale_stride = size_t(N) * (K / 32);
  uint3 local_tid = uint3(0, tid.y, 0);
  const device T* source = x + size_t(source_row) * K;
  device T* output = y + size_t(row) * (2 * N);
  fp_qmv_fast_impl<T, 32, 4>(
      w0 + size_t(expert) * weight_stride,
      scales0 + size_t(expert) * scale_stride,
      source,
      output,
      K,
      N,
      ...
      simd_lane_id);
  fp_qmv_fast_impl<T, 32, 4>(
      w1 + size_t(expert) * weight_stride,
      scales1 + size_t(expert) * scale_stride,
      source,
      output + N,
      K,
      N,
...
#define instantiate_deepseek_mxfp4_masked_pair(type)                          \
  template [[host_name("deepseek_mxfp4_gather_masked_pair_qmv_" #type)]]     \
  [[kernel]] decltype(deepseek_mxfp4_gather_masked_pair_qmv<type>)            \
      deepseek_mxfp4_gather_masked_pair_qmv<type>

instantiate_deepseek_mxfp4_masked_pair(float16_t);
instantiate_deepseek_mxfp4_masked_pair(bfloat16_t);
